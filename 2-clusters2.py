...
for i in range(0, len(sub_where)-1):
	f.write(f"{df.loc[sub_where[i], 'id']}\n")
	if sub_where[i+1] - sub_where[i] > 1:
		f.write(f"{df.loc[sub_where[i]+1, 'id']}\n")
f.write(f"{df.loc[sub_where[-1], 'id']}\n{df.loc[sub_where[-1]+1, 'id']}\n")
f.close()

f2 = open(filename2, "a")
for i in range(0, len(sub_where)-1):
	f2.write(df.loc[sub_where[i], "aminoacid"] + df.loc[sub_where[i], "anticodon"] + df.loc[sub_where[i], "strand"] + "::" + str(sub[i]) + "::")
	if sub_where[i+1] - sub_where[i] > 1:
		f2.write(df.loc[sub_where[i]+1, "aminoacid"] + df.loc[sub_where[i]+1, "anticodon"] + df.loc[sub_where[i]+1, "strand"] + "\n")
f2.write(df.loc[sub_where[-1], "aminoacid"] + df.loc[sub_where[-1], "anticodon"] + df.loc[sub_where[-1], "strand"] + "::" + str(sub[-1]) + "::" + df.loc[sub_where[-1]+1, "aminoacid"] + df.loc[sub_where[-1]+1, "anticodon"] + df.loc[sub_where[-1]+1, "strand"] + "\n")
f2.close()